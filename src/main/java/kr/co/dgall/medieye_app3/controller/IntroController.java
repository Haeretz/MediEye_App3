...
public class IntroController {
	...
	public ApiResponse<?> getIntro(@RequestBody(required = false) IntroRequest request){
		log.info("request: {}", request.getDeviceId()+request.getCurrentVer()+request.getPlatform());
		if(request.getDeviceId() == null || request.getCurrentVer() == null || request.getPlatform() == null){
		...
		Boolean checkVer = introService.checkVersion(request.getCurrentVer());
